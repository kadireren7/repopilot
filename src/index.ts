...
async function runAnalyze(target: string): Promise<void> {
  if (target.startsWith("http://") || target.startsWith("https://")) {
    throw new RepoPilotError(
      "Remote URL analysis is not available in this release.",
      "REMOTE_ANALYZE_UNSUPPORTED",
      "Clone the repository locally and run: repopilot analyze <local-path>"
    );
  }
  ...
  const scoring = new ScoringEngine().calculate(analysis);
...
async function runFix(target: string, dryRun: boolean, overwrite: boolean): Promise<void> {
  if (target.startsWith("http://") || target.startsWith("https://")) {
    throw new RepoPilotError(
      "Remote fix is not available in this release.",
      "REMOTE_FIX_UNSUPPORTED",
      "Use a local repository path and rerun with --dry-run first."
    );
  }
...
void yargs(hideBin(process.argv))
  ...
  .command(
    "analyze <target>",
    "Analyze a local repository for ship-readiness",
    (cmd) =>
